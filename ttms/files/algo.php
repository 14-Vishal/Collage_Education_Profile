...
for ($i = 3; $i < 9; $i += 2) {
    for ($k = 0; $k < 7; $k++) {
        for ($j = 0; $j < 6; $j++) {
            ...
            if (isset($subjectslots[$i][$k][$j % 6])) {
            } else {
                $subjectslots[$i][$k][$j % 6] = "-";
                $aliasslots[$i][$k][$j % 6][0] = "-";
            }
...
for ($I = 0; $I < 2 * $count; $I++) {
    ...
    for ($j = 0; $j < 7; $j++) {
        if (isset($subjectslots[$sem][$j][6])) {
            continue;
        } else if (isset($teachers[$tindex]->days[$sem % 2][$j][6]) || isset($teachers[$tindex2]->days[$sem % 2][$j][6]) ||
            isset($teachers[$tindex3]->days[$sem % 2][$j][6])
        ) {
            continue;
        } else {
...
for ($i = 0; $i < $teachers_count; $i++) {
    for ($k = 0; $k < 7; $k++) {
        ...
        if (isset($teachers[$i]->days[1][$k][6])) {}
        else {
            $teachers[$i]->days[1][$k][6] = "-";
        }
    }
}
...
practical='" . $subjectslots[$i][$j][6] . "<br>" . $aliasslots[$i][$j][6][0] . ", " . $aliasslots[$i][$j][6][1] . ", " . $aliasslots[$i][$j][6][2] . "'
 WHERE day='" . $days[$j] . "' ";
...
practical='" . $teachers[$i]->days[1][$j][6] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"), $query);
    }
}
