    ...
    if (isset($_POST['teacherexcel'])) {
        if (empty($_FILES['file']['tmp_name'])) {
            ...
            while (!feof($handle)) {
                ...
                $faculty_number = $filesop[0];
                $name = $filesop[1];
                $alias = $filesop[3];
                $designation = $filesop[4];
                $contact = $filesop[5];
                $email = $filesop[6];
                if ($faculty_number == "" || $faculty_number == "Faculty No.") {
                    continue;
                }
                $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
                    "INSERT INTO teachers VALUES ('$faculty_number','$faculty_name','password','$alias','$designation','$contact','$email')");
                if ($q) {
