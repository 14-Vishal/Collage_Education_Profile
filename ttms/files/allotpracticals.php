...
Security
Insights
Timetable-Management-System/TTMS/files/allotpracticals.php /
@msaqib4203
msaqib4203 Add files via upload
Latest commit f39c882 on Feb 26, 2017
 History
 1 contributor
293 lines (269 sloc)  10.6 KB
    ...
    <div align="center" style="margin-top: 5px">
        <select name="toalloted" class="list-group-item">
         ...
         <option selected disabled>Select Teacher</option>';
                while ($row = mysqli_fetch_assoc($q)) {
                    $mystring .= '<option value="' . $row['faculty_number'] . '">' . $row['faculty_name'] . '</option>';
                }
...
<table id=allotedpracticalstable>
    ...
    while ($row = mysqli_fetch_assoc($q)) {
        ...
        $t1 = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
            "SELECT faculty_name FROM teachers WHERE faculty_number = '$teacher_id1'");
        $trow1 = mysqli_fetch_assoc($t1);
        $t2 = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
            "SELECT faculty_name FROM teachers WHERE faculty_number = '$teacher_id2'");
        $trow2 = mysqli_fetch_assoc($t2);
        $t3 = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
            "SELECT faculty_name  FROM teachers WHERE faculty_number = '$teacher_id3'");
        ...
        echo "<tr><td>{$row['subject_code']}</td>
                    ...
                    <td>{$trow2['faculty_name']}</td>
                    <td>{$row['allotedto3']}</td>
                    <td>{$trow3['faculty_name']}</td>
                   <td>
